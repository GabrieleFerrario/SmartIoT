...
// alert
int lower_bound_light;
int upper_bound_light;
int lower_bound_temperature;
int upper_bound_temperature;
int lower_bound_rssi;
...
void loop() {
  ...
  if (show == 0) {
    ...
    if ((millis() - lastTime_home) > timerDelay && flag_home) {
  ...
  if ((millis() - lastTime_weather) > timerDelay && flag_weather) {
      String jsonBuffer;
      ...
      wind = doc["wind"]["speed"];
      json["city"] = city;
      json["weather_condition"] = weather;
    ...
    // display screen
    if(flag_home & !flag_weather & (millis() - lastTime2_home) > timerDelay){
      
      showHome(int(h), hic, rssi, lightSensorValue);
      lastTime2_home = millis();
      
    } else if (!flag_home & !flag_weather){
    ...
    else if (!flag_home && flag_weather & (millis() - lastTime2_weather) > timerDelay){
      
      showWeather(weather, temperature, wind);
      lastTime2_weather = millis();
      led_external_status = HIGH;
      digitalWrite(LED_EXTERNAL, led_external_status);
      led_external2_status = HIGH;
      digitalWrite(LED_EXTERNAL2, led_external2_status);
      ...
      // toggles the content of the display
...
void connectToMQTTBroker() {
  if (!mqttClient.connected()) {   // not connected
    ...
    mqttClient.subscribe("gferrario/activeSlaveWeather");
    Serial.println(F("\nSubscribed to gferrario/activeSlaveWeather topic!"));
    mqttClient.subscribe("gferrario/activeSlaveHome");
    Serial.println(F("\nSubscribed to gferrario/activeSlaveHome topic!"));
...
void mqttMessageReceived(String &topic, String &payload) {
  ...
  if (topic == "gferrario/activeSlaveWeather"){ 
     // Slave Weather
  ...
  if (topic == "gferrario/activeSlaveHome"){ 
      // Slave Home
      // verify the identity of the web app request
      ...
      if(doc["slave"] == "NodeMCUHome"){
        ...
        doc_slave["lower_bound_light"] = doc["lower_bound_light"];
        doc_slave["upper_bound_light"] = doc["upper_bound_light"];
        doc_slave["lower_bound_temperature"] = doc["lower_bound_temperature"];
        doc_slave["upper_bound_temperature"] = doc["upper_bound_temperature"];
        doc_slave["lower_bound_rssi"] = doc["lower_bound_rssi"];
  ...
  if (topic == "gferrario/serveSlave"){ 
    ...
    if( res != ""){
      ...
      if((!flag_home & flag_weather & res == "home") || (flag_home & !flag_weather & res == "weather") || (!flag_home & !flag_weather))
  ...
  if(topic == "gferrario/deactiveSlaveHome"){ 
    // disable the slave and update the master on the new status
...
void showWeather(String weather, float temperature, float wind){
  // weather monitoring screen for display
